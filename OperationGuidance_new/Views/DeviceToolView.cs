...
{
    public class DeviceToolView: ACustomDataGridViewOuterPanel<DeviceToolDTO, ViewObjects.DeviceToolVO> {
        #region Fields
        // Apis
        ...
        private void OpenEditEntityPopUpForm(string title, DeviceToolDTO dto, Action callBackAction) {
            ...
            // 添加字段
            CustomTextBoxGroup name = _editEntityPopUpForm.AddTextBox("工具名称", false, 
            ...
            confirmButton.Click += (s, e) => {
                ...
                string warningMsg = "";
                int warningIndex = 1;
                List<DeviceToolDTO> allData = apis.QueryDeviceToolList(new(SystemUtils.MacAddressesDTO.id)).DeviceToolDTOs;
                if (string.IsNullOrEmpty(name.GetTextBox(0).Box.Text)) {
                    check = false;
                    name.GetTextBox(0).IsError = true;
                    warningMsg += $"{warningIndex++}. 工具名称不能为空\r\n";
                }
                if (allData.Exists(d => d.id != dto.id && d.name == dto.name)) {
                    check = false;
                    name.GetTextBox(0).IsError = true;
                    warningMsg += $"{warningIndex++}. 工具名称已存在\r\n";
                }
                ...
                if (type.IsDefaultValue()) {
                    type.SetError(true);
                    check = false;
                    warningMsg += $"{warningIndex++}. 没有选择工具类型\r\n";
                }
                if (!check) {
        ...
        protected override List<DeviceToolVO> QueryList() {
            QueryDeviceToolListRsp rsp = apis.QueryDeviceToolList(new(SystemUtils.MacAddressesDTO.id));
