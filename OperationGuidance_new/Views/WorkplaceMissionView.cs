...
namespace OperationGuidance_new.Views
{
    public class WorkplaceMissionView: CustomContentPanel {
        ...
        public WorkplaceMissionView() : base() {
            ...
            MissionListPanel.RightButton toWorkplaceButton = new();
    ...
    public class TopBar: CustomMainMenuPanel {
        ...
        private void RecalculateTitle() {
            ...
            if (this.Width < this.Height) {
                this.Font = new(WidgetsConfigs.SystemFontFamily, (int) (Math.Ceiling(this.Width / 70F) + 2), FontStyle.Bold);
            } else {
                this.Font = new(WidgetsConfigs.SystemFontFamily, (int) (Math.Ceiling(this.Height / 2.3F) + .4), FontStyle.Bold);
            }
    ...
    public class WorkplaceContent: CustomContentPanel {
        ...
        private CustomTextBoxGroup _barCodeTextBox;
        ...
        private void InitializeLeftTop() {
            ...
            _barCodeTextBox = new("", ConfigsVariables.COLOR_TEXT_BOX_BORDER_ERROR) {
                Parent = _leftTop,
            ...
            TextBox box = _barCodeTextBox.GetBox(0);
            ...
            void barCodePopUp(object? s, EventArgs e) {
                if (_barCodePopUpForm == null) {
                    _barCodePopUpForm = new(note, box) {
                        Title = "录入条码",
                ...
                if (box.Text != note) {
                    _barCodePopUpForm.TextBox.SetValue(0, box.Text);
                }
        ...
        private void ReSizeContents() {
            ...
            int bottomHeight = (int) (wholeHeight * .1);
        ...
        private void ResizeleftTop() {
            // 输入框、icon距离边框的尺寸
            int padding = (int) (_leftTop.Height * .135);
            // icon的边长
            int side = _leftTop.Height - padding * 2;
            // 重设icon
            _barCodePictureBox.Image = WidgetUtils.ResizeImageWithoutLosingQuality(_barCodeImage, side, side);
            _barCodePictureBox.Margin = new(padding);
            _barCodePictureBox.Size = new(side, side);

            // 重设输入框
            int newH = (int) (side * .95);
            _barCodeTextBox.Size = new(_leftTop.Width - _leftTop.Height - padding * 2, newH);
            _barCodeTextBox.Margin = new(padding, (_leftTop.Height - newH) / 2, 0, 0);
            ...
            if (_barCodePopUpForm != null) {
                ...
                int popUpW = (int) (mainW * .55);
                int popUpH = (int) (mainH * .12 + mainW * .03);
        ...
        private void ResizeRightMiddle() {
            // Reset font size depends on parent szie
            _torqueTitle.Font = new Font(WidgetsConfigs.SystemFontFamily, Width / 180 + Height / 60 + 1, FontStyle.Bold);
            _angleTitle.Font = new Font(WidgetsConfigs.SystemFontFamily, Width / 180 + Height / 60 + 1, FontStyle.Bold);
            // Resize titles
            _torqueTitle.Size = new(_rightMiddle.Width - 2, (int) (_torqueTitle.Font.Height * 1.3));
            _angleTitle.Size = new(_rightMiddle.Width - 2, (int) (_angleTitle.Font.Height * 1.3));
            // Resize data text
        ...
        private void ResizeRightBottom() {
            // Reset font size depends on parent size
            _productSideTitle.Font = new Font(WidgetsConfigs.SystemFontFamily, Width / 180 + Height / 60 + 1, FontStyle.Bold);
            // Resize title
            _productSideTitle.Size = new(_rightBottom.Width - 2, (int) (_productSideTitle.Font.Height * 1.3));
    ...
    public class BarCodeInputPopUpForm: CustomPopUpForm {
        ...
        public CustomTextBoxGroup TextBox {
            get => this._textBox;
            set => this._textBox = value;
        }

        public BarCodeInputPopUpForm(string initStr, TextBox upperBox) : base() {
            _initStr = initStr;
            _textBox = new("", ConfigsVariables.COLOR_TEXT_BOX_BORDER_ERROR) {
                Parent = this,
                BorderColor = ConfigsVariables.COLOR_TEXT_BOX_BORDER,
                BoxBackColor = ConfigsVariables.COLOR_TEXT_BOX_BACKGROUND,
                BoxForeColor = ConfigsVariables.COLOR_TEXT_BOX_FOREGROUND,
            };
            TextBox textBox = _textBox.GetBox(0);
            textBox.GotFocus += (s, e) => {
                EventFuncs.CurrentActiveControl = textBox;
                if (textBox.Text == _initStr) {
                    textBox.Text = "";
                }
            };
            textBox.LostFocus += (s, e) => {
                if (textBox.Text == "") {
                    textBox.Text = _initStr;
                }
            };
            textBox.Focus();
    ...
    public class WorkingProcessPanel: Panel {
        ...
        private void InvokeResizing() {
            ...
            _statusTxt.Font = new(WidgetsConfigs.SystemFontFamily, Height * .285F, FontStyle.Bold, GraphicsUnit.Pixel);
            _statusTxt.Size = new(Width - _borderSize * 2, _statusTxt.Font.Height);
            _statusTxt.Location = new(_borderSize, (Height - _statusTxt.Height) / 2);
            _statusDesc.Font = new(WidgetsConfigs.SystemFontFamily, (Height - _picturePanel.Height) * .28F, FontStyle.Bold, GraphicsUnit.Pixel);
            _statusDesc.Size = new(Width - _borderSize * 2, _statusDesc.Font.Height);
            int hTemp = _picturePanel.Location.Y + _picturePanel.Height;
            _statusDesc.Location = new(_borderSize, hTemp + (Height - hTemp - _borderSize - _statusDesc.Height) / 2);
        }
