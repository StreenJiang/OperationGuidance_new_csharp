...
namespace OperationGuidance_service.Database {
    public class SqlServerConnector: ADbConnector {
        ...
        public override DbConnection? GetDbConnection() {
            try {
                SqlConnection conn = new($"server={Server}; port={Port}; database={Database}; user={User}; password={Password}; charset=UTF8; Connection Timeout=2;");
                conn.Open();

                string sqlScriptPrefix = "modify_sqlserver";
                if (!ConnectionUtils.CheckTableExists(conn, new UserAccountInfo().TableName())) {
                    using (SqlCommand command = conn.CreateCommand()) {
                        command.CommandText = ConnectionUtils.GetInitializationSql("init_sqlserver", sqlScriptPrefix);
