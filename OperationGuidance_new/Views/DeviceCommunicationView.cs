...
namespace OperationGuidance_new.Views {
    public class DeviceCommunicationView: CustomDataGridViewOuterPanel<DeviceCommunicationDTO, ViewObjects.DeviceCommunicationVO> {
        ...
        private List<DeviceCommunicationDTO> _dataDTOList;
        // DataGridView panel
        private DataGridViewGroup<DeviceCommunicationVO> _dataGridView;
        // Add new pop up form
        private EditEntityPopUpForm<DeviceCommunicationDTO> _editEntityPopUpForm;
        #endregion

        #region Constructors
        public DeviceCommunicationView() {
            // Default values
        ...
        private void InitializeGridView() {
            ...
            _dataGridView.AddTextBox("设备名称", false, (DeviceCommunicationVO vo, string? value) => vo.name = value);
            _dataGridView.AddTextBox("设备描述", false, (DeviceCommunicationVO vo, string? value) => vo.description = value);
            CustomComboBoxGroup<int?> toolTypeComboBox = _dataGridView.AddComboBox("设备类型", (DeviceCommunicationVO vo, int? value) => vo.type = value, new());
            foreach (DeviceTypeBase type in DeviceType_Communication.Elements) {
                toolTypeComboBox.AddItem(type.Name, type.Id);
            }
            
            // 按钮逻辑
            _dataGridView.QueryData = (vo) => {
                List<DeviceCommunicationVO> workstationVOs = QueryList();
            ...
            _dataGridView.AddNewClick = (action) => {
                DeviceCommunicationDTO dto = new();
            ...
            _dataGridView.ModifyClick = (ids, action) => {
                if (ids.Count <= 0) {
                    ...
                    if (_dataDTOList.Count > 0) {
                        DeviceCommunicationDTO dto = _dataDTOList.Single(dto => dto.id == ids[0]);
        ...
        private void OpenEditEntityPopUpForm(string title, DeviceCommunicationDTO dto, Action callBackAction) {
            ...
            CustomTextBoxGroup name = _editEntityPopUpForm.AddTextBox("设备名称", false, 
                (DeviceCommunicationDTO dto, string? value) => dto.name = value ?? "");
            if (dto.name != null) {
            ...
            CustomTextBoxGroup description = _editEntityPopUpForm.AddTextBox("设备描述", false, 
                (DeviceCommunicationDTO dto, string? value) => dto.description = value ?? "");
            ...
            confirmButton.Click += (s, e) => {
                ...
                string warningMsg = "";
                int warningIndex = 1;
                List<DeviceCommunicationDTO> allData = apis.QueryDeviceCommunicationList(new(SystemUtils.MacAddressesDTO.id)).DeviceCommunicationDTOs;
        ...
        protected override List<DeviceCommunicationVO> QueryList() {
            QueryDeviceCommunicationListRsp rsp = apis.QueryDeviceCommunicationList(new(SystemUtils.MacAddressesDTO.id));
