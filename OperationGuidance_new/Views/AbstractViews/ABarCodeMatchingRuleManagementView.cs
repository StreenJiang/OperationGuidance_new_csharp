...
namespace OperationGuidance_new.Views.AbstractViews {
    public abstract class ABarCodeMatchingRuleManagementView<V>: CustomDataGridViewOuterPanel<BarCodeMatchingRuleDTO, V> where V : BarCodeMatchingRuleVO, new() {
        ...
        // DataGridView panel
        protected DataGridViewGroup<V> _dataGridView;
        // Add new pop up form
        protected EditEntityPopUpForm<BarCodeMatchingRuleDTO> _editEntityPopUpForm;
        protected List<ProductMissionDTO> _missions;
        protected CustomComboBoxGroup<int?> _missionNameComboBox;
        #endregion

        #region Constructors
        public ABarCodeMatchingRuleManagementView() {
            // Default values
        ...
        private void InitializeGridView() {
            ...
            CustomComboBoxGroup<int?> barCodeTypeComboBox = _dataGridView.AddComboBox("条码类型", (V vo, int? value) => vo.type = value, new());
            foreach (BarCodeType type in BarCodeTypes.Elements) {
                barCodeTypeComboBox.AddItem(type.Name, type.Id);
            }
            _missionNameComboBox = _dataGridView.AddComboBox("应用任务", (V vo, int? value) => vo.mission_id = value, new());
            RefreshMissionOptions();

            // 按钮逻辑
            _dataGridView.QueryData = (vo) => {
                List<V> vos = QueryList();
                return vos
        ...
        protected void ResizePopUpForm() {
        ...
        protected override List<V> QueryList() {
            ...
            List<V> vos = new();
            CommonUtils.ObjectConverter<BarCodeMatchingRuleDTO, V>(_dataDTOList, vos);

            // Set mission names
            foreach (ProductMissionDTO mission in _missions) {
                List<V> vosTemp = vos.Where(v => v.mission_id == mission.id).ToList();
