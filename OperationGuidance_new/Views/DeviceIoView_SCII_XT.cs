...
namespace OperationGuidance_new.Views {
    public class DeviceIoView_SCII_XT: CustomDataGridViewOuterPanel<DeviceIoDTO, DeviceIoVO> {
        #region Fields
        // Apis
        private OperationGuidanceApis apis;
        private List<DeviceIoDTO> _dataDTOList;
        // DataGridView panel
        private DataGridViewGroup<DeviceIoVO> _dataGridView;
        // Add new pop up form
        private EditEntityPopUpForm<DeviceIoDTO> _editEntityPopUpForm;
        #endregion

        #region Constructors
        public DeviceIoView_SCII_XT() {
            // Default values
        ...
        private void InitializeGridView() {
            ...
            _dataGridView.AddTextBox("IO设备名称", false, (DeviceIoVO vo, string? value) => vo.name = value);
            _dataGridView.AddTextBox("IO设备描述", false, (DeviceIoVO vo, string? value) => vo.description = value);
            CustomComboBoxGroup<int?> toolTypeComboBox = _dataGridView.AddComboBox("IO设备类型", (DeviceIoVO vo, int? value) => vo.type = value, new());
            ...
            _dataGridView.AddNewClick = (action) => {
                DeviceIoDTO dto = new();
                OpenEditEntityPopUpForm("新增IO设备", dto, action);
            };
            _dataGridView.ModifyClick = (ids, action) => {
                if (ids.Count <= 0) {
                    ...
                    if (_dataDTOList.Count > 0) {
                        DeviceIoDTO dto = _dataDTOList.Single(dto => dto.id == ids[0]);
                        OpenEditEntityPopUpForm("更新IO设备信息", dto, action);
                    }
                }
            };
        ...
        private void OpenEditEntityPopUpForm(string title, DeviceIoDTO dto, Action callBackAction) {
            ...
            confirmButton.Click += (s, e) => {
                ...
                string warningMsg = "";
                int warningIndex = 1;
                List<DeviceIoDTO> allData = apis.QueryDeviceIoList(new(SystemUtils.MacAddressesDTO.id)).DeviceIoDTOs;
                if (string.IsNullOrEmpty(name.GetTextBox(0).Box.Text)) {
                    check = false;
                    name.GetTextBox(0).IsError = true;
                    warningMsg += $"{warningIndex++}. IO设备名称不能为空\r\n";
                }
                if (allData.Exists(d => d.id != dto.id && d.name == dto.name)) {
                    check = false;
                    name.GetTextBox(0).IsError = true;
                    warningMsg += $"{warningIndex++}. IO设备名称已存在\r\n";
                }
        ...
        protected override void AddOrUpdate(DeviceIoDTO dto, Action action) {
            AddOrUpdateDeviceIoRsp rsp = apis.AddOrUpdateDeviceIo(new(dto));
        ...
        protected override void Delete(List<int> ids) {
            if (ids.Count <= 0) {
                ...
                DeleteDeviceIoByIdsRsp rsp = apis.DeleteDeviceIo(new(ids));
                if (rsp.RsponseCode == HttpResponseCode.OK) {
